...
std::unique_ptr<LogicalPlan> Planner::getBestPlan(const BoundStatement& statement) {
    ...
    case StatementType::QUERY: {
        ...
        //         if (relExpr != nullptr) {
        //             std::cout << relExpr->getSrcNode()->toString() << ", " <<
        //             extend_string(relExpr->getExtendDirections().at(0)) << ", "<<
        //             relExpr->getDstNode()->toString() << std::endl;
        ...
        //             binder::Expression* expr = info.pattern.get();
        //             std::cout << (expr->dataType).toString() << "," << expr->getUniqueName() <<
        //             ", " << expr->getAlias() << std::endl;
        //         }
        ...
        outfile.close();
        // std::cout << "best plan is \n" + planner::LogicalPlanUtil::encodeJoin(*plan) <<
        // std::endl;
    } break;
