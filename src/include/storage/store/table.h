...
//     // Node/Rel ID vector. We assume all output vectors are within the same DataChunk as this
//     one. common::ValueVector* nodeIDVector; std::vector<common::ValueVector*> outputVectors;
...
//     TableScanState(std::vector<common::column_id_t> columnIDs, std::vector<const Column*>
//     columns)
//         : table{nullptr}, nodeIDVector(nullptr), outState{nullptr},
...
public:
    ...
    // virtual void insert(transaction::Transaction* transaction, TableInsertState& insertState) =
    // 0; virtual void update(transaction::Transaction* transaction, TableUpdateState& updateState)
    // = 0; virtual bool delete_(transaction::Transaction* transaction, TableDeleteState&
    // deleteState) = 0;
...
protected:
    // virtual bool scanInternal(transaction::Transaction* transaction, TableScanState& scanState) =
    // 0;
