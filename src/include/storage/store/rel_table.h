...
public:
    ...
    RelTable(catalog::RelTableCatalogEntry* relTableEntry, const StorageManager* storageManager,
        ...
        : Table{relTableEntry, storageManager, memoryManager}, fromNodeTableID{relTableEntry->getSrcTableID()},
          toNodeTableID{relTableEntry->getDstTableID()}, nextRelOffset{0} {}
    ...
    // bool scanInternal(transaction::Transaction* transaction, TableScanState& scanState)
    // override;

    // void insert(transaction::Transaction* transaction, TableInsertState& insertState)
    // override; void update(transaction::Transaction* transaction, TableUpdateState&
    // updateState) override; bool delete_(transaction::Transaction* transaction,
    // TableDeleteState& deleteState) override;

    // void detachDelete(transaction::Transaction* transaction, common::RelDataDirection
    // direction,
    ...
    // void commit(transaction::Transaction* transaction, catalog::TableCatalogEntry*
    // tableEntry,
    //     LocalTable* localTable) override;
    // void checkpoint(common::Serializer& ser, catalog::TableCatalogEntry* tableEntry)
    // override; void rollbackCheckpoint() override {}; void reclaimStorage(FileHandle&
    // dataFH) override;
